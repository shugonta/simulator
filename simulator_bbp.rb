...
=end
  ...
  if traffic_list.size > 0
    traffic_list[0].each { |traffic_item|
        ...
        if o.match(/^Model has been successfully generated$/) != nil
          ...
          if o2.match(/Integer optimal/) != nil
            ...
            route.each { |route_item|
              if route_item != nil && route_item.size > 0
                route_cnt = route_cnt.succ
                write_log2(route_item[0].bandwidth.to_s + ": ")
                route_reliability = 1
                route_item.each { |link|
                  write_log2(sprintf("(%d->%d), ", link.start_node, link.end_node))
            ...
            write_log2(sprintf("[ExpectedBandwidth-MinCostFlow(%d-%d)] %f\n", traffic_item.id, i, expected_bandwidth.to_s))
            ...
            if max_not_achived_bandwidth == 0
              ...
              bandwidth = (max_not_achived_bandwidth + bandwidth) /2
              min_achived_bandwidth = bandwidth
            end
          end
        else
          #   モデル異常
      ...
      if final_route != nil
        write_log2(sprintf("[Accepted(%d)] %d->%d (%d, %f)\n", traffic_item.id, traffic_item.start_node, traffic_item.end_node, final_bandwidth, traffic_item.quality))
        # ルート使用処理
        route_cnt = 0
        active_traffic = ActiveTraffic.new(time + traffic_item.holding_time, traffic_item.clone, [])
        final_route.each { |route_item|
          if route_item != nil && route_item.size > 0
            route_cnt = route_cnt.succ
            route_item.each { |link|
